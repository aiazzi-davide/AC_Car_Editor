...
- [x] Implementare gestione rapporti cambio (gear ratios) - Editor con spinbox per ogni marcia (R, 1-10), import da libreria con 4 preset, stima velocità massima per ogni marcia
...
- [x] implementare modifiche a cartella ui/ (nome auto in menu, icone, etc.) - UIManager + UIEditorDialog per modificare ui_car.json (nome, brand, descrizione, tags, specs, author)
- [x] implementare possibilità per rendere un auto stage 1/2/3 con un click: con differenze tra auto turbo e NA (es. se NA stage 1 = mappa più aggressiva, stage 2 = aggiunta turbo, stage 3 = aggiunta turbo + modifiche meccaniche, se turbo stage 1 = aumento boost, stage 2 = aumento boost, stage 3 = aumento boost + modifiche meccaniche, aerodimanica, etc.) - StageTuner + StageTuningDialog implementati con logic differenziata NA/Turbo
- [x] Implementare sistema undo/redo modifiche (tramite file .bak ?) è sufficiente poter ripristinare un bakup dell'ultima modifica - bottone "↩ Restore Backup" nel CarEditorDialog; CarFileManager.restore_bak_files() copia tutti i .bak sull'originale, elimina i .bak e ricarica l'editor
