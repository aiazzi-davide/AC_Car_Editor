...
class MainWindow(QMainWindow):
    ...
    def on_car_selected(self, current, previous):
        ...
        # If only data.acd exists, it will be auto-unpacked when editing
        ...
        if car_info['has_data_acd'] and car_info['has_data_folder']:
            ...
            self.statusBar.showMessage(
                f"Car '{car_name}' has no data folder or data.acd. Cannot edit."
            )
        else:
    ...
    def edit_car(self):
        ...
        if not os.path.exists(car_data_path):
            QMessageBox.warning(
                ...
                "The car may not have any data files to edit."
            )
            return
        ...
        # Refresh car info display after editing
